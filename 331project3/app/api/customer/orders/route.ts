...
export async function POST(request: Request) {
    ...
    try {
        ...
        const emailToSave = (customerEmail && customerEmail.includes('@')) ? customerEmail : null;
        ...
        const insertOrderQuery = `
            INSERT INTO orders (order_timestamp, total_amount, customer_email, order_status)
            VALUES (NOW(), $1, $2, 'PLACED')
            RETURNING order_id
        `;
        const orderResult = await client.query(insertOrderQuery, [totalAmount, emailToSave]);
